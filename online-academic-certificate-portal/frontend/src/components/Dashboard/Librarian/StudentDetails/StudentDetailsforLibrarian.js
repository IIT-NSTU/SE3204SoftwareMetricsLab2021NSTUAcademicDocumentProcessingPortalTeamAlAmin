...
import { librarian_accept_provisional } from "../../../../actions/auth";
import './StudentDetails.css';

const StudentDetailsforLibrarian = () => {
    ...
    const acceptLibrarian = (email) => {
        console.log(email)
        disptach(librarian_accept_provisional(email))
    }

    const rejectLibrarian = () => {
        if (certificateData.student_details) {
            navigate(`/librarian/${data.roll}/reject`)
        }
    }
            ...
            {certificateData.student_details &&
                ...
                <React.Fragment >
                    ...
                    <div style={{ textAlign: 'center', marginTop: '30px' }}>
                        ...
                        <input type="submit" value="Accept" id="accept" onClick={() => acceptLibrarian(certificateData.student_details.email)}
                            style={{ width: '15%', marginLeft: 'auto' }}
                            disabled={certificateData.librarian_status === "approved"}
                        />



                        <input type="submit" value="Reject" id="reject"
                            onClick={rejectLibrarian}
...
export default StudentDetailsforLibrarian